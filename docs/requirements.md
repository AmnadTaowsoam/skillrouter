...
SkillRouter is a skill routing engine for AI platform teams, agent builders, support automation teams. The product must support the complete path from setup to reviewed output, with enough traceability and operational control for real use by a small team.

## 2. Goals

- Deliver one reliable end-to-end workflow for skill routing engine.
...
- intent classifier, skill registry, policy gates, context selection, routing audit trail
...
- The product must provide a guided intake form for skill routing engine.
...
- The product should support at least one integration handoff in MVP: MCP servers, Codex skills, internal knowledge bases, observability.
...
Primary risk areas: wrong tool routing, privilege escalation, missing fallback, opaque decisions.
