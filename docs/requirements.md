# SkillRouter Requirements

## 1. Purpose

SkillRouter is a Skill / Tool / Agent Selection Engine that helps users produce practical, reviewable, and reusable outputs from a focused workflow. The product must be useful as a public repository while still being compatible with Cerebra MCP as an internal orchestration, review, testing, security, and DevOps layer.
...
- Name: SkillRouter
- Category: Skill / Tool / Agent Selection Engine
- Summary: A router-first agent system that chooses the right skill, tool, or agent for a task.
- Priority: Phase 3 / platform
- Cerebra MCP fit: CerebraSkill-mcp, CerebraRole-mcp, CerebraOrchestrator-mcp, CerebraSecurity-mcp

## 3. Goals

- Provide a complete MVP path from input to usable output.
- Make SkillRouter understandable to public GitHub visitors without requiring private Cerebra context.
...
The person who runs SkillRouter to create or inspect the target artifact. They need clear setup, fast feedback, and usable output.
...
- Output must match the Skill / Tool / Agent Selection Engine use case.
...
Main risk areas: wrong tool routing, privilege escalation, opaque decisions, missing fallback, over-routing
...
- a new user can understand SkillRouter from README.md
